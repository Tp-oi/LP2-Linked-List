...
int main() {
  ...
  problemTwoHeadNode = problemTwoCurrentNode;  // Set the head of the list to the first node.
...
void removeNode(node*& head, int deletePosition) {  // Function to remove a node at a specific position in the linked list.
  if (deletePosition == 1) {                        // Check if the node to delete is the first node.
    node* nextNode = head;                          // Store the current head node in a temporary pointer.
  ...
  for (int i = 1; i < deletePosition - 1; i++) {  // Traverse the list to find the node before the one to delete.
    currentNode = currentNode->ptrNextNode;       // Move to the next node.
  }

  node* deleteNode = currentNode->ptrNextNode;         // Store the node to delete in a temporary pointer.
  currentNode->ptrNextNode = deleteNode->ptrNextNode;  // Update the current node's pointer to skip the node to delete.
...
node* addNode(node* currentNode,
  ...
  if (currentNode == NULL) {          // Check if the current node is NULL (i.e., the list is empty or we are adding the first node)
    node* currentNode = new node;     // Create a new node dynamically
    currentNode->ptrNextNode = NULL;  // Set the pointer of the new node to NULL (indicating the end of the list)
    currentNode->info = i;            // Assign the provided value 'i' to the 'info' field of the new node
    return currentNode;               // Return the newly created node
  }

  else {
    node* newNode = new node;            // Create a new node dynamically
    currentNode->ptrNextNode = newNode;  // Link the current node's 'ptrNextNode' to the newly created node
    newNode->ptrNextNode = NULL;         // Set the pointer of the new node to NULL (indicating the end of the list)
    newNode->info = i;                   // Assign the provided value 'i' to the 'info' field of the new node
